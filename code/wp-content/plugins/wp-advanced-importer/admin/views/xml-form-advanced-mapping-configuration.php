...
if($_POST) {
   if($_REQUEST['istemplate'] == 'no'){
      ...
      $post_values = $xml_uci_admin->GetPostValues(sanitize_key($_REQUEST['eventkey']));
      $result = array_merge($post_values[$_REQUEST['eventkey']], $records);
      $xml_uci_admin->SetPostValues(sanitize_key($_REQUEST['eventkey']), $result);
   }
}
...
if($istemplate == 'no'){
   ...
   $actionURL = esc_url(admin_url() . 'admin.php?page=smack-uci-import&step=import_config&istemplate=no&eventkey='.$_REQUEST['eventkey']);
} else {
   $backlink = esc_url(admin_url() . 'admin.php?page=smack-uci-import&step=import_config&istemplate=no&eventkey='.$_REQUEST['eventkey']);
   $actionURL = esc_url(admin_url() . 'admin.php?page=smack-uci-import&step=import_config&eventkey='.$_REQUEST['eventkey']);
}
$templateName = ''; $template_mapping = array();
if(isset($_REQUEST['templateid'])) {
   $templateInfo = $wpdb->get_results($wpdb->prepare("select templatename, mapping from wp_ultimate_csv_importer_mappingtemplate_xml where id = %d", $_REQUEST['templateid']));
...
if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'edit') {
   $templateInfo = $wpdb->get_results($wpdb->prepare("select templatename, mapping from wp_ultimate_csv_importer_mappingtemplate_xml where eventKey = %s", $_REQUEST['eventkey']));
   $template_mapping = maybe_unserialize($templateInfo[0]->mapping);
   $templateName = $templateInfo[0]->templatename;
}
...
if (isset($_GET['tag_name']) && $_GET['tag_name'] != "") {
  ...
  if(isset($_REQUEST['istemplate']) && sanitize_title($_REQUEST['istemplate']) == 'no') {
    $backlink = esc_url(admin_url() . 'admin.php?page=smack-uci-import&step=mapping_config&istemplate=no&eventkey='.$_REQUEST['eventkey']);
  }
  else{
    $backlink = esc_url(admin_url() . 'admin.php?page=smack-uci-import&step=mapping_config&eventkey='.$_REQUEST['eventkey']);
  }
  if(isset($_REQUEST['templateid'])) {
  $backlink .= '&templateid=' . intval($_REQUEST['templateid']);
  }
...
<!--                                    </tr>-->
      ...
      if(!isset($_REQUEST['action'])) { ?>
         <div align="center" style="font-size: 15px;" class="col-md-12">
            ...
            <?php } else { ?>
               ...
               <input type='text' class="form-control" id='templatename' name ='templatename' style='margin-left:10px; width:25% !important; display:none;' placeholder='<?php echo $templateName; ?>' value='<?php echo $templateName;?> '/>
            <?php } ?>
         </div>
      <?php } ?>
      <?php if(isset($_REQUEST['action']) && sanitize_title($_REQUEST['action']) == 'edit') { ?>
         ...
         <input type="button" class="smack-btn smack-btn-primary btn-radius mapping_continuebtn" value="<?php echo esc_attr__('Save','wp-advanced-importer');?>" style="margin-bottom:5%;" onclick="update_template('<?php echo sanitize_key($_REQUEST['eventkey']);?>');">
      <?php } else { ?>
         <div class="col-md-12 mt20">
            <!-- back btn -->
            <div class="pull-left">
            ...
            <!--continue btn -->
            <div class="pull-right">
               <input type="button" class="smack-btn smack-btn-primary btn-radius" value="<?php echo esc_attr__('Continue','wp-advanced-importer');?>" onclick="save_template('<?php echo $import_type;?>');">
            </div>
         </div>
         <!-- continue btn end -->
         <div class="clearfix"></div>
